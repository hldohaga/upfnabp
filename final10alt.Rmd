---
title: "Exposure to High Ultra-processed Food(UPF) and Sodium Intake and its effect on Hypertension from a Cross-sectional study  UK National Dietary and Nutritional Survey (NDNS) in England 2008-2019"
output:
  word_document: 
    toc: yes
    fig_caption: yes
...
search terms (("ultraprocessed food*" OR "ultra-processed food*" OR "ultra processed food*" OR "NOVA food*")OR (salt OR Sodium OR "Sodium intake")) AND ( "blood pressure" OR hypertension OR "cardiovascular disease" OR "cardiovascular risk")AND(cohort OR cross-section\* OR prospective OR meta-analysis OR "systematic review")

Inclusion prospective and observational studies and systematic reviews specifically on hypertension with relevant exposures"
...
"Scopus search"=c("18/6/23","240"), "Medline(Ovid)Search" =c("18/6/23","202"),"Pubmed search"=c("18/6/23","103"),"Web of Science Search" =c("18/6/23","48"),"Total"=c("18/6/23","593")
...
searchcht <- grViz(diagram = "digraph flowchart {
  ...
  [4]: 'Systematic Review and UPF 3' 
  [5]: 'Systematic Review and Sodium 4' 
  [6]: 'Eligible Studies 11'
  ")
print(searchcht)
searchcht
...
Their systematic review identifies Monge et al @monge2021 , Scaranni @ScaranniPatriciadeOliveiradaSilva2021Ufci , and Mendonça @DeDeusMendonçaRaquel2017Ufca.
...
All participants who responded were included.
...
Weighting values are then used to standardise the sample for age, sex, IMD, and nation.
...
The cut off values were therefore 0-3g, 3-5g, 5-6g, and over 6g.
...
The value for each NOVA group was developed by comparing every food level entry in NDNS against NOVA using standard methodology described by Colombet and Chavez-Ugalde @colombet2023. Another approach has been published by Martinez-Steele et al. @martinez-steele2023 .
...
Wang @WangMei2022UFCI used cut off values which are low for the UK.
I used centre-weighted categories, plus/minus 1 standard deviations from the mean.
This effectively identifies 67% in the central category.
...
Hypertension is BP over 140 mmHg @williams2018 , categorised to enable logistic regression.
...
Categorizing into two categories enables a comparison to be made using logistic regression generating an odds ratio describing the relationship between the hypertension and not hypertension.
...
In the analysis I have used the quintile categories of IMD with each nations IMD categories treated as comparable.
This may introduce some inaccuracy as the four nations use different baselines at different cut off dates, but should be accurate enough for the purpose of this analysis.
...
The whole population and the analysed population were described.
The populations in each sub group were described.

The exposure and outcome variables were mapped across the years.
Non-standardised values from different years were compared to identify differences between the years.
...
Multivariable regression models were constructed to manage explanatory variables which might have confounding effects on the outcome of the analysis.
These variables were Age, Sex, IMD, BMI, age at completion of education and year of survey.
...
Akaike index statistics(AIC) were used to assess 'goodness of fit' of logistic models.
R squared does not work for logistic regression.
A relative effect size was calculated by comparison of AIC statistics.
...
keydatw1 <-tbl_svysummary(ndns_1_11d, include= c(Sex,Age ))
#keydatw1
keydatwdd <-tbl_svysummary(ndns_1_11dd, include= c(Sex,Age  ) )

keydatwzpf <-tbl_svysummary(ndns_1_11zpf, include= c(Sex,Age) )
keydatwna <-tbl_svysummary(ndns_1_11na, include= c(Sex,Age ) )
keydatwhyp <-tbl_svysummary(ndns_1_11hyp, include= c(Sex,Age) )
...
Univariable linear regression is adjusted for weighted survey samples.
Identified important relationships within the data.

Understanding these results may help explain confounding relationships in the data.

UPF compared to Sodium shows a zero beta value indicating no linear relationship.

Linear regression of UPF shows a negative relationship with BP, which is statistically significant, beta = -0.19 (CI -0.22,-0.15).
...
There is also a positive relationship of age with Sodium, which is also statistically significant 1.5 (CI 0.77,2.3).
...
They were regressed against hypertension in participants.
...
The Akaike Information Criterion (AIC), a measure of 'goodness of fit', is lower for this combined model, 3590.81, indicating that it better fits the data.
...
In a combined model the analysis shows a statistically significant odds ratio of 5.57(1.47,21.2) for hypertension with Sodium intake between 5g and 6g.
Between high UPF and hypertension there was an odds ratio of 0.57(0.34,0.94) hypertension with high UPF intake .
Multivariable logistic regression models all included age, sex, BMI, IMD, age at leaving education, and survey year.

The effect is also present in the multivariable logistic regression model for separate variables, for Sodium this is OR=5.20(CI 1.39,19.5), and for UPF OR=0.60(CI 0.36,0.99).
...
These results show statistical significance because of the statistical power due to the large sample size.

Sub-group size and heterogeneity are reasons for caution over the overall results.
...
### Not simply additive effect

The regression model including both exposure variables had a better fit of data than either individually.
This finding suggests that the effect of each variable is not the whole effect.
...
The finding of a supplementary effect, and to understand the types of UPF ingested in this population would be priorities.
