...
ndns_1_11m <- subset(ndns_1_11dd, Sex=="Male")
ndns_1_11m <- subset(ndns_1_11m, Age <=18)
ndns_1_11f <- subset(ndns_1_11f, Age <= 18)
ndns_1_11na <- subset(ndns_1_11m, Na >= 5000)
#ndns_1_11upf<- subset(ndns_1_11dd, UPF >= 63)
ndns_1_11zpf<- subset(ndns_1_11m, NOVA_Epct_opti_4 >= 63)
ndns_1_11hyp <- subset(ndns_1_11m, BP >= 140)
...
keydatw1 <-tbl_svysummary(ndns_1_11m, include= c(Sex,Age, agegad3,   educfinh, IMD,region,SurveyYear ))
#keydatw1
keydatwdd <-tbl_svysummary(ndns_1_11m, include= c(Sex,Age, agegad3, educfinh, IMD,region,SurveyYear ) )
...
ttestage <- svyglm(Age ~ SurveyYear, ndns_1_11m)
ttestages<-  add_global_p(tbl_regression(ttestage))
ttestbmi <- svyglm(bmival~SurveyYear, ndns_1_11m)
...
#sexchi <- svychisq(~Sex +SurveyYear , ndns_1_11m)
educhi <- svychisq(~educfinh+SurveyYear , ndns_1_11m[,"SurveyYear" !=5])
imdchi <- svychisq(~IMD+SurveyYear , ndns_1_11m)
vegchi <- svychisq(~vegetarn+ SurveyYear , ndns_1_11m)
UPF3chi <- svychisq(~zPF3+ SurveyYear , ndns_1_11m)
agegad3chi <- svychisq(~agegad3+ SurveyYear , ndns_1_11m)
hypchi <- svychisq(~hyp+ SurveyYear , ndns_1_11m)
BMIchi <- svychisq(~BMI+ SurveyYear, ndns_1_11m)
gorchi <- svychisq(~region+ SurveyYear, ndns_1_11m)
tablecat<- data.table("Variable"=c(  "IMD","Age","BMI","Region"),"p.value"= c(imdchi$p.value,agegad3chi$p.value,BMIchi$p.value,gorchi$p.value ))
...
keydatw <-tbl_svysummary(ndns_1_11m,by =SurveyYear , include = c(hiNa,zPF3))
...
svyboxplot(zPF~SurveyYear,ndns_1_11m, na.rm = TRUE,main = "UPF% by survey year", ylab = "UPF%" ,xlab ="Survey Year")
```
...
svyboxplot(Na~SurveyYear,ndns_1_11m,main = "Sodium Intake in mg by survey year", ylab = "Sodium intake in mg" ,xlab ="Survey Year")
...
svyplot(BP~SurveyYear,ndns_1_11m, na.rm = TRUE,main = "BP in mmHg by survey year", ylab = "BP in mmHg" ,xlab ="Survey Year")
```
...
tblbpdata <-tbl_svysummary(ndns_1_11m,by =SurveyYear ,include = c(hyp))
...
hyp1chi <- svychisq(~hyp+ hiNa2 , ndns_1_11m, na.rm=TRUE)
#hyp1chi$p.value
#round(ftable(hyp1chi$observed),2)


hyp2chi <- svychisq(~hyp+ zPF2, ndns_1_11m, na.rm=TRUE)
#hyp2chi$p.value
...
#UPF Energy % only
lm1E <- svyglm(BP ~ zPF, ndns_1_11m)
lm1Et <- tbl_regression(lm1E)
#univariable regression models
#Sodium only
lm1na <- svyglm(BP ~ Na, ndns_1_11m)
lm1nat <- tbl_regression(lm1na)

lmEna <- svyglm(zPF ~ Na, ndns_1_11m)
lmEnat <- tbl_regression(lmEna)

lmA <- svyglm(BP ~ Age, ndns_1_11m)
lmAt <- tbl_regression(lmA)

lmEA <- svyglm(zPF ~ Age, ndns_1_11m)
lmEAt <- tbl_regression(lmEA)

lmNaA <- svyglm(Na ~ Age, ndns_1_11m)
...
lm1AS <- svyglm(hyp ~  BMI +agegad3+educfinh + IMD +SurveyYear,family = binomial(link = "logit"), ndns_1_11m , na.action = na.exclude)

theme_gtsummary_mean_sd()

#Sodium only
lmna <- svyglm( hyp ~ hiNa  +agegad3 + BMI+educfinh+  IMD+SurveyYear ,family = binomial(link = "logit"), ndns_1_11m,na.action = na.exclude)

lmnaE <- svyglm(hyp ~ hiNa  +agegad3+ zPF3 + BMI +educfinh+ IMD +SurveyYear,family = binomial(link = "logit"), ndns_1_11m,na.action = na.exclude)

lmME <- svyglm( hyp~ zPF3 +agegad3+ BMI +educfinh + IMD +SurveyYear,family = binomial(link = "logit"), ndns_1_11m,na.action =na.exclude)
