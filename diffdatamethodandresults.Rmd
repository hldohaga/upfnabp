...
sav4 <- "/media/david/Elements1/contents/DOHmed/MPH/MPHModules/dissertation/alternativequant/upfnabp/data/UKDA-6533-tab/tab/ndns_rp_yr1-4a_personleveldietarydata_uk_v2.tab"

sav4b <- "/media/david/Elements1/contents/DOHmed/MPH/MPHModules/dissertation/alternativequant/upfnabp/data/UKDA-6533-tab/tab/ndns_rp_yr1-4a_indiv_uk.tab"

sav6 <-"/media/david/Elements1/contents/DOHmed/MPH/MPHModules/dissertation/alternativequant/upfnabp/data/UKDA-6533-tab/tab/ndns_rp_yr5-6a_personleveldietarydata_v2.tab"

sav6b <- "/media/david/Elements1/contents/DOHmed/MPH/MPHModules/dissertation/alternativequant/upfnabp/data/UKDA-6533-tab/tab/ndns_rp_yr5-6a_indiv.tab"

sav8 <-"/media/david/Elements1/contents/DOHmed/MPH/MPHModules/dissertation/alternativequant/upfnabp/data/UKDA-6533-tab/tab/ndns_rp_yr7-8a_personleveldietarydata.tab"

sav8b <- "/media/david/Elements1/contents/DOHmed/MPH/MPHModules/dissertation/alternativequant/upfnabp/data/UKDA-6533-tab/tab/ndns_rp_yr7-8a_indiv.tab"



sav11 <- "/media/david/Elements1/contents/DOHmed/MPH/MPHModules/dissertation/alternativequant/upfnabp/data/UKDA-6533-tab/tab/ndns_rp_yr9-11a_personleveldietarydata_uk_20210831.tab"

sav11b <- "/media/david/Elements1/contents/DOHmed/MPH/MPHModules/dissertation/alternativequant/upfnabp/data/UKDA-6533-tab/tab/ndns_rp_yr9-11a_indiv_20211020.tab"

sav4r <- as.data.table(read.table(sav4,sep = "\t",header = TRUE))
sav4br <-as.data.table(read.table(sav4b,sep = "\t",header = TRUE))

sav6r <- as.data.table(read.table(sav6,sep = "\t",header = TRUE))
sav6br <-as.data.table(read.table(sav6b,sep = "\t",header = TRUE))

sav8r <- as.data.table(read.table(sav8,sep = "\t",header = TRUE))
sav8br <-as.data.table(read.table(sav8b,sep = "\t",header = TRUE))


sav11r <- as.data.table(read.table(sav11,sep = "\t",header = TRUE))
sav11br <- as.data.table(read.table(sav11b,sep = "\t",header = TRUE))
...
summary(sav6br[ seriali >=50000000 & seriali <=60000000 ,educfinh])
...
summary(ndns_1_11[SurveyYear == 11,omsysval])
...
The continuous variables age and bmi, when compared across the Survey Years show no significant differences, table x.
...
tablecon
```
Now analysing the categorical data in table 42.
There is no difference in sex,age of finishing education, Eimd, are not statistically significant.
...
Simple linear regression equations look for the relationship between the Systolic BP and UPF or Sodium.
For these I am looking at the whole dataset 

```{r echo=FALSE}
svyplot(omsysval~Sodiummg, ndns_1_11d ,main = "Sodium Intake in mg by Systolic BP", xlab = "Sodium Intake mg" ,ylab ="Systolic BP")
...
First, omsysval is compared to EnergykJ, then sodiummg and the sodium intake compared with the energy intake.
...
as_flex_table(tbl_stack(list(row1,row2),group_header = c("bp","age")))
```
...
Regression of UPF with age shows an equal effect to that on BP.
...
This first model (lm1AS) in table x looks at the relationships between BP and Age and Sex education and IMD all of which may have an effect on BP. This model excludes UPF and Na.
...
The next model (lmna) adds Sodiummg.
...
```
This second model gives Sodiummg, educfinh, and IMD statistical significance, alongside Age and Sex.



Now we add UPF (lmnaE) as total energy from nova 4 or UPF
...
then removing sodiummg

```{r echo=FALSE, warning=FALSE}
lmME <- svyglm(omsysval ~ Age + Sex+Epcnt_4 + bmival +educfinh + EIMD_2010_quintile , ndns_1_11d,na.action =na.exclude)
#anova(lmME)
```
the UPF continues to have no significance, despite this being present in the univariable regression. 
This suggests that the effect of UPF is affected by the other variables in the model.

comparing AIC for these three models 

```{r echo=FALSE, message=FALSE, warning=FALSE}

lm1ASt <-add_global_p( tbl_regression(lm1AS, show_single_row = c(Sex)))
lmnat <- add_global_p(tbl_regression(lmna, show_single_row = c(Sex)))
lmnaEt <-add_global_p( tbl_regression(lmnaE, show_single_row = c(Sex)))
lmMEt <- add_global_p(tbl_regression(lmME, show_single_row = c(Sex)))
