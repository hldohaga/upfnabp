...
To work out the amount of food intake by each individual, first the diary entries for each individual are totalled up.
...
anova(lm1E11)
```
There is a relationship between Total EMJ and omsysval with a negative gradiant.
...
lastfew11 <- sav11rp[,.(Age, Sex,Sodiummg,pcnt_4, omsysval, omdiaval)]
lastfew4 <- sav4rp[,.(Age, Sex,Sodiummg,pcnt_4, omsysval, omdiaval)]
alldata <- data.table(rbind(lastfew4,lastfew11))

lmall <- lm(omsysval~ Age+ Sex + Sodiummg +pcnt_4, alldata)
lmall
anova(lmall)
```
