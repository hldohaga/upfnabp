---
title: "Exposure to High Ultra-processed Food and Sodium Intake and its effect on Hypertension using the UK National Dietary and Nutritional Survey (NDNS) in England 2008-2019"
output:
  html_notebook: 
    toc: yes
    fig_caption: yes
...
# Exposure to High Ultra-processed Food and Sodium Intake and its effect on Hypertension using the UK National Dietary and Nutritional Survey (NDNS) in England 2008-2019 {.unnumbered}
...
This is a secondary data study the national dietary and nutrition survey @universityofcambridge2022 which looked at the risk of hypertension on exposure to high intake of UPF and Na.
...
\newpage
```
```{r tbl-abbreviations, echo=FALSE, message=TRUE, warning=TRUE}
#| tab-id: "table 1"
...
This study looked at this association in the data set of the National Dietary and Nutrition Survey (NDNS) (@universityofcambridge2022 ).The study gives data from the UK from 2008 to 2019.
It is representative of the population by sex, age, region and index of multiple deprivation (IMD).
It recorded exposure to high UPF and high Sodium intake, and outcome BP.

This study assesses correlation between BP and exposure to these dietary factors.
...
The aim of CHAMPs in Cheshire and Merseyside is shared by public health across the world.
BP is an indicator of the health of the population, it is a risk factor for a number of non-communicable diseases (NCD).
...
If UPF and sodium intake increase the risk of hypertension they may be on the pathway, or mechanism of action, of social and commercial determinants.
Then policy to reduce exposure might deliver change at a population level.
...
Using PICO I am able to frame the research question.

In adults and children across the four home nations of the UK between 2008 and 2019, did exposure to high sodium dietary intake, and or high UPF dietary intake increase the odds of having a mean systolic blood pressure of over 140mmHg?
...
What was the risk relationship between these?
...
3 Analysis for correlation between UPF, Na and BP using regression models.
...
table2 <- set_caption(table2, "Table of Search terms used", 
...
NDNS is a rolling cohort study in each year a new cohort of participants is enrolled.
Using questionnaires, food diaries, and nurse assessments to gather data.
It has been running since 2008.
The latest data is available from 2019.
...
Participants were identified by random selection across postal units, stratified to ensure a representative sample across the four nations (England, Wales, Scotland, and Northern Ireland) and across regions in England (North West, North East, Yorkshire and Humberside, East Midlands,West Midlands, London, The South East, The South West).
The sample is also representative for age and sex and IMD.

All participants in NDNS are included in this analysis.
...
The sodium value (Na) was calculated from intake.
Food diaries were anlaysed against standard food nutrient values.
Hence, this value reflects the expected Na content of standard foods.It assumes the content remains consistent.
...
A standard methodology for this has been published more recently by .
...
Social desirability bias acknowledges that participants remember and record intake framed by their beliefs about the needs of the study, and their bliefs about what is percieved as being healthy.
...
Differing weights are available for different levels of analysis as participants who did not complete the initial interview were not selected for subsequent blood analysis.
...
A sample size calculation for this secondary analysis is available in appendix 1 the initial proposal from OpenEpi @openepi.
...
An odds ratio of 1.2 was used based on a meta-analysis by Wang et al @wang.
...
Analysis was by regression, with univariable and multivariable linear regression as well as logistic regression used.
...
keydatw1 <-tbl_svysummary(ndns_1_11d, include= c(Sex,Age, agegad3,  bmival,  educfinh, IMD,gor ))
#keydatw1
keydatwdd <-tbl_svysummary(ndns_1_11dd, include= c(Sex,Age, agegad3,  bmival, educfinh, IMD,gor ) )

keydatwzpf <-tbl_svysummary(ndns_1_11zpf, include= c(Sex,Age, agegad3,  bmival,  educfinh, IMD,gor) )
keydatwna <-tbl_svysummary(ndns_1_11na, include= c(Sex,Age, agegad3,  bmival,  educfinh, IMD,gor ) )
keydatwhyp <-tbl_svysummary(ndns_1_11hyp, include= c(Sex,Age, agegad3,  bmival,  educfinh, IMD,gor) )
...
table3 <- set_caption(table3, "Characteristics of the sample population from National Dietary and Nutrition Study (2008-2019) N= 15,655", 
...
## Descriptive Data: Time, Place, Person

NDNS was conducted over 11 years so far, beginning in 2008 [figure 4](fig:fig-survey-year).

```{r}
#| fig-id: "fig-survey-year"
#| fig-cap: "Histogram for each annual cohort NDNS(2008-2019)"
svyhist(~SYear , ndns_1_11dd )
```

NDNS was conducted across the four nations of the UK [figure 5](fig:fig_gor).

```{r fig_gor, echo=FALSE, message=FALSE, warning=FALSE}
#| fig-id: "fig_gor"
#| fig-cap: "Histogram for region NDNS(2008-2019)"
fig_gor <-svyhist(~ngor, ndns_1_11dd)
fig_gor
```

NDNS included male and female participants above the age of 1.5 years.

```{r fig-Age, echo=FALSE, message=FALSE, warning=FALSE}
#| fig-id: "fig-Age"
#| fig-cap: "Histogram for Age NDNS(2008-2019)"
svyhist(~Age , ndns_1_11dd)
```

In the study population The median age was 38.
...
## Exposure variables by Sex, Age and Place

The [table 4.1.1](tab:table3) shows the population exposed to UPF \>63% of their calories is made up of 4793 participants.
...
They are also younger than average, 61% are 19-35.
...
#keydatw <-tbl_svysummary(ndns_1_11dd,by =SurveyYear , include = c(Na,UPF,BP))
...
svyboxplot(NOVA_Epct_opti_4~SurveyYear,ndns_1_11dd, na.rm = TRUE,main = "UPF% by survey year", ylab = "UPF%" ,xlab ="Survey Year")
```
...
keydatwx <-tbl_svysummary(ndns_1_11dd,Sex, include = c(Na,zPF,zPF3,BP))
...
keydatwA <-tbl_svysummary(ndns_1_11dd,agegad3, include = c(Na,zPF,zPF3,BP))
...
keydatwp <-tbl_svysummary(ndns_1_11dd,gor, include = c(Na,zPF,zPF3,BP))
...
Exposed/unexposed vs outcome yes/no

```{r}
hyp1chi <- svychisq(~hyp+ hiNa , ndns_1_11dd)
hyp2chi <- svychisq(~hyp+ zPF3, ndns_1_11dd)
```
...
lmnat <- tbl_regression(lmna,include=hiNa,exponentiate = TRUE)
lmnaEt <- tbl_regression(lmnaE,include=c(hiNa,zPF3),exponentiate = TRUE)
lmMEt <- tbl_regression(lmME,include=zPF3,exponentiate = TRUE)

multivariables<- set_caption(as_flex_table(  tbl_merge(list(lmnat,lmMEt,lmnaEt), tab_spanner = c("Na only","UPF only","Na and UPF"))), caption = "Table of multivariable regression against BP to identify the effects relating to Na and UPF NDNS data 2008-2019")
multivariables <- add_footer_lines(multivariables, "All models include Sex,Age, BMI, Education, IMD and Survey Year")
...
```{r bp-multivariable-modelsf, echo=FALSE, warning=FALSE}

lm1ASf <- svyglm(hyp ~  BMI +agegad3+educfinh + IMD +SurveyYear,family = binomial(link = "logit"), ndns_1_11f , na.action = na.exclude)

theme_gtsummary_mean_sd()

#sodium only
lmnaf <- svyglm( hyp ~ hiNa   +agegad3 + BMI+educfinh+  IMD+SurveyYear ,family = binomial(link = "logit"), ndns_1_11f,na.action = na.exclude)

lmnaEf <- svyglm(hyp ~ hiNa  +agegad3+ zPF3 + BMI +educfinh+ IMD +SurveyYear,family = binomial(link = "logit"), ndns_1_11f,na.action = na.exclude)

lmMEf <- svyglm( hyp~ zPF3 +agegad3 + BMI +educfinh + IMD +SurveyYear,family = binomial(link = "logit"), ndns_1_11f,na.action =na.exclude)
...
```{r tbl-multivariable-outputs-bpf, echo=FALSE, message=FALSE, warning=FALSE}
#| tab-id: "tbl-multivariable-outputs-bpf"
#| tbl-cap: "Table of multivariable regression against BP to identify the effects relating to Na and UPF NDNS data 2008-2019"
#lm1ASt <-tbl_regression(lm1AS,include=c(hiNa,NOVA_Epct_opti_4) ,exponentiate = TRUE)
lmnaft <- tbl_regression(lmnaf,include=hiNa,exponentiate = TRUE)
lmnaEft <- tbl_regression(lmnaEf,include=c(hiNa,zPF3),exponentiate = TRUE)
lmMEft <- tbl_regression(lmMEf,include=zPF3,exponentiate = TRUE)

multivariablesf<- set_caption(as_flex_table(  tbl_merge(list(lmnaft,lmMEft,lmnaEft), tab_spanner = c("Na only","UPF only","Na and UPF"))), caption = "Table of multivariable regression against BP to identify the effects relating to Na and UPF NDNS data 2008-2019")
multivariablesf <- add_footer_lines(multivariables, "All models include Sex,Age, BMI, Education, IMD and Survey Year")
