...
#| tab-id: tbl-abbreviations
#| caption: "Table of abbreviations used"

abbrev <- data.table("Abbreviation "=c("NDNS","BP","Na","UPF","NCD","CVD","CHAMPs","NOVA","NAFLD","BMI"),"Term"=c("National Dietary and Nutrition Survey","Blood Pressure","Sodium intake in mg", "Ultra Processed Foods", "Non communicable Disease","Cardiovascular Disease","Cheshire and Merseyside public health  collaborative","NOVA is a classification system, it is not an acronym","non- alcohol fatty liver disease","Body Mass Index"))
width(autofit(flextable(abbrev)), width=5)
```
...
This study explored this complex web pulling out strands within it, \@ref(fig:figdiag1) shows a possible arrangement of this.
...
Jafar @jafar2018 argues that understanding the position of the investgator be of interest to understanding this quantitative study.
...
My search terms are included in \@ref(tab:tbl-search-terms) below.
...
ndns_1_11f <- subset(ndns_1_11d, Sex == "Female")
ndns_1_11na <- subset(ndns_1_11d, Na >= 5000)
ndns_1_11upf<- subset(ndns_1_11d, UPF >= 63)
ndns_1_11zpf<- subset(ndns_1_11d, NOVA_Epct_opti_4 >= 63)
ndns_1_11hyp <- subset(ndns_1_11d, BP >= 140)
...
The separate waves had separate participants.The data is presented in \@ref(tab:tbl-keydata) tbl-keydata.
...
\@ref(fig:fig-upf-and-survey-year )s howed overlap between the waves for UPF intake.
@ref(fig:fig-Na-and-survey-year) showed Na exposure overlap between waves.
@ref(fig:fig-BP-and-survey-year) identified the lack of data for year 11.
...
@ref(tab:tbl-Key-Variables-by-Survey-year) shows results for continuous variables.
...
@ref(tab:tbl-Categorical-variables-year) follows.
...
@ref(tab:tbl-keydatax) shows the difference between male and female in the key variables.
...
@ref(tab:tbl-tbl-keydataA) shows the age distribution of exposure to sodium and UPF and the resulting BP.
...
@ref(tab:tbl-keydatap) shows the data.
...
The regression model for Sodium against BP shows that there is no linear relationship between Sodium and BP in this table @ref(tab:tbl-univariable-regressions).
...
\@ref(tab:tbl-multivariable-outputs-bp) follows below.[![]()](tbl-multivariable-outputs-bp)
...
The subsequent \@ref(tbl-AIC-comparison) shows the size of the effect relating to sodium.
