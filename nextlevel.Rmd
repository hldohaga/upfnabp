...
This first table @tbl-table1 includes all variables.
The NDNS population is 15,655, with 1,438 taking medication removed for column two.
...
```{r tbl-table1, echo=FALSE, message=FALSE, warning=FALSE,cache=TRUE}
#| label: tbl-table1
#| tbl-cap: "Characteristics of the sample population from National Dietary and Nutrition Study (2008-2019) N= 15,655"


keydatw1 <-tbl_svysummary(ndns_1_11d, include= c(Age, agegad3,Sex, omsysval,hyp, bmival, Na,hiNa, UPF,UPF3, educfinh, EIMD_2010_quintile,gor,bpd ))
#keydatw1
keydatwdd <-tbl_svysummary(ndns_1_11dd, include= c(Age, agegad3,Sex, omsysval,hyp, bmival, Na,hiNa, UPF,UPF3, educfinh, EIMD_2010_quintile,gor,bpd ) )
...
```{r tbl-setup, include=FALSE,echo=FALSE,message=FALSE}
...
svyboxplot(Epcnt_4~SurveyYear,ndns_1_11d, na.rm = TRUE,main = "Energy from UPF in percent by survey year", ylab = "Energy percent from UPF" ,xlab ="Survey Year")
```
...
svyboxplot(Sodiummg~SurveyYear,ndns_1_11d,main = "Sodium Intake in mg by survey year", ylab = "Sodium intake in mg" ,xlab ="Survey Year")
...
#svyboxplot(omsysval~SurveyYear,ndns_1_11d, na.rm = TRUE,main = "Systolic BP in mmHg by survey year", ylab = "Sysltolic BP in mmHg" ,xlab ="Survey Year")
```
...
ttestNa2 <- svyglm(Na~SurveyYear, ndns_1_11d )
ttestNa2t <- tbl_regression(ttestNa2)
ttestEpcnt <- svyglm(UPF~SurveyYear, ndns_1_11d )
ttestEpcntt <- tbl_regression(ttestEpcnt)
ttestOS <- svyglm(BP~SurveyYear, ndns_1_11d)
...
as_flex_table(tbl_stack(list(ttestNa2t,ttestEpcntt,ttestOSt,ttestages,ttestbmis),group_header =  c("Sodium in mg", "Percent Energy UPF","Systolic BP","Age","BMI")))
```
...
agegad3chi <- svychisq(~agegad3+ SurveyYear , ndns_1_11d)
hypchi <- svychisq(~hyp+ SurveyYear , ndns_1_11d)

tablecat<- data.table("Variable"=c( "Sex", "Education","IMD","Vegetarian","UPF3","Age","Hypertension"),"ChiSq"=c(sexchi$statistic,educhi$statistic,imdchi$statistic,vegchi$statistic,UPF3chi$statistic,agegad3chi$statistic,hypchi$statistic ),"p.value"= c(sexchi$p.value,educhi$p.value,imdchi$p.value,vegchi$p.value,UPF3chi$p.value,agegad3chi$statistic,hypchi$statistic ))
...
keydatwx <-tbl_svysummary(ndns_1_11d,Sex, include = c(Na,UPF,UPF3,BP))
...
keydatwA <-tbl_svysummary(ndns_1_11d,agegad3, include = c(Na,UPF,UPF3,BP))
...
keydatwp <-tbl_svysummary(ndns_1_11d,gor, include = c(Na,UPF,UPF3,BP))
...
#UPF Energy % only
lm1E <- svyglm(BP ~ UPF, ndns_1_11d)
lm1Et <- tbl_regression(lm1E)
#univariable regression models
#sodium only
lm1na <- svyglm(BP ~ Na, ndns_1_11d)
lm1nat <- tbl_regression(lm1na)

lmEna <- svyglm(UPF ~ Na, ndns_1_11d)
lmEnat <- tbl_regression(lmEna)

lmA <- svyglm(BP ~ Age, ndns_1_11d)
lmAt <- tbl_regression(lmA)

lmEA <- svyglm( Age ~ UPF, ndns_1_11d)
lmEAt <- tbl_regression(lmEA)

lmNaA <- svyglm(Na ~ Age, ndns_1_11d)
...
#univariable regression models
#UPF Energy % only
lm1bmi <- svyglm(BP ~ bmival, ndns_1_11d)
lm1bmit <- tbl_regression(lm1bmi)
#univariable regression models
#sodium only
lm1Ag1 <- svyglm(BP ~ agegad3, ndns_1_11d)
lm1Ag1t <- tbl_regression(lm1Ag1)

lmEbmi <- svyglm(UPF ~ bmival, ndns_1_11d)
lmEbmit <- tbl_regression(lmEbmi)

lmed <- svyglm(BP ~ educfinh, ndns_1_11d)
lmedt <- tbl_regression(lmed)

lmEs <- svyglm(UPF ~ agegad3, ndns_1_11d)
lmEst <- tbl_regression(lmEs)

lmEed <- svyglm(UPF ~ educfinh, ndns_1_11d)
lmEedt <- tbl_regression(lmEed)

lmNabm <- svyglm(Na ~ bmival, ndns_1_11d)
lmNabmt <- tbl_regression(lmNabm)

lmNaed <- svyglm(Na ~ educfinh, ndns_1_11d)
lmNaedt <- tbl_regression(lmNaed)

lmNaAg <- svyglm(Na ~ agegad3, ndns_1_11d)
lmNaAgt <- tbl_regression(lmNaAg)

#row1<- tbl_merge(list(lm1nat,lm1Et,lmEnat),tab_spanner = c("BP","UPF","Na"))
#row2 <- tbl_merge(list(lmAt,lmEAt,lmNaAt),tab_spanner = c("BP","UPF","Na"))
as_flex_table(tbl_stack(list(lm1nat,lmEnat,lm1Et,lmEAt,lmAt,lmNaAt,lm1bmit,lm1Ag1t,lmedt,lmEbmit,lmEst,lmEedt,lmNabmt,lmNaAgt,lmNaedt),group_header = c("BP/Na","UPF/Na","UPF/bp","UPF/Age","Age/BP","Age/Na","BP/bmi","BP/Agg1","BP/ed","UPF/bmi","UPF/age","UPF/ed","Na/bmi","Na/Agg","Na/ed")))
```
...
lm1AS <- svyglm(hyp ~  Sex +bmival +agegad3+educfinh + EIMD_2010_quintile+bpd ,family = binomial(link = "logit"), ndns_1_11d , na.action = na.exclude)

theme_gtsummary_mean_sd()

#sodium only
lmna <- svyglm( hyp ~ hiNa  +Sex +agegad3 + bmival+educfinh+  EIMD_2010_quintile+bpd ,family = binomial(link = "logit"), ndns_1_11d,na.action = na.exclude)

lmnaE <- svyglm(hyp ~ hiNa +Sex +agegad3+UPF3 + bmival +educfinh+ EIMD_2010_quintile +bpd ,family = binomial(link = "logit"), ndns_1_11d,na.action = na.exclude)

lmME <- svyglm( hyp~ UPF3 +agegad3+Sex + bmival +educfinh + EIMD_2010_quintile +bpd,family = binomial(link = "logit"), ndns_1_11d,na.action =na.exclude)
...
lmMEt <- tbl_regression(lmME,exponentiate = TRUE)
 ...
 as_flex_table(  tbl_merge(list(lm1ASt,lmnat,lmMEt,lmnaEt), tab_spanner = c("No Na or UPF","Na only","UPF only","Na and UPF")))
```
...
```{r AIC-sense1, echo=TRUE, message=FALSE, warning=FALSE}
