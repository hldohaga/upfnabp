...
first4t <- tbl_summary( subset(ndns_1_11,SurveyYear <=4 ),include = c(Sodiummg,Epcnt_4,pcnt_4,omsysval),
                 statistic = list(all_continuous() ~"{mean} ({sd})"))

first4tw <-tbl_svysummary(first4, include = c(Sodiummg,Epcnt_4,pcnt_4,omsysval),
                 statistic = list(all_continuous() ~"{mean} ({sd})"))
...
last3t <- tbl_summary(subset(ndns_1_11,SurveyYear >= 9 ), include = c(Sodiummg,Epcnt_4,pcnt_4,omsysval),
                 statistic = list(all_continuous() ~"{mean} ({sd}"))
 last3tw <- tbl_svysummary(last3,include = c(Sodiummg,Epcnt_4,pcnt_4,omsysval),
                 statistic = list(all_continuous() ~"{mean} ({sd})"))
last3t
last3tw
```
...
#ttestEkJ <- svyglm(EnergykJ_4~(SurveyYear >=5), ndns_1_11d )
#ttestEkJ


tbl_merge(list(ttestNa2t,ttestEpcntt))
...
```{r echo=FALSE}
#tablesex <- svyglm(~Sex+SurveyYear,design = ndns_1_11d,family = "binomial")
#tbl_regression(tablesex)
#sexchi <- svyglm(~SurveyYear+Sex , ndns_1_11ed)
#tbl_regression(sexchi)
#ttestage$p.value
#sexchi$p.value
```
...
```{r echo=FALSE}
tableveg <- svytable(~vegetarn +SurveyYear, ndns_1_11d)
tableveg
# svyglm(vegetarn,SurveyYear,family = binomial(link = "logit"),ndns_1_11d)
...
stargazer(lm1na,lm1E,title="Results", align=TRUE, type = "text")
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
lmM2 <- svyglm(omsysval ~ Age +Sex +Sodiummg + Epcnt_4+ bmival +educfinh+ EIMD_2010_quintile , ndns_1_11d,na.action = na.exclude)
...
lmMna <- svyglm(omsysval ~ Age +Sex + Epcnt_4+ bmival +educfinh + EIMD_2010_quintile  , ndns_1_11d,na.action =na.exclude)


tbl_regression(lmMna)
#lmMna$coefficients
...
lmMt <- tbl_regression(lmM)
lmM2t <- tbl_regression(lmM2)
lmMnat <- tbl_regression(lmMna)

tbl_merge(list(lm1ASt,lmMt,lmM2t,lmMnat))



#stargazer(lm1AS,lmM,lmMna,lmM2,title = "multi", align = FALSE, type = "text" )
