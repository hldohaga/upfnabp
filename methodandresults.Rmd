...
```{r fig-dagupfbp2, echo=FALSE, message=FALSE, warning=FALSE}
#| label: fig-dagupfbp2
#| fig-cap: "DAG of relationships explored by analysis"
...
The data is summarised for the key continuous variables by survey year cohort.
...
A wider set of variables will be included in the backgorund and tested to ensure they have no affect on the results to be demonstrated. 
...
```{r adjusting-dag, echo=FALSE, message=FALSE, warning=FALSE}
 for( n in names(bp1) ){
     for( m in setdiff( descendants( bp1, n ), n ) ){
         a <- adjustmentSets( bp1, n, m )
         if( length(a) > 0 ){
 ...
 for( n in names(bp1) ){
     for( m in children(bp1,n) ){
         iv <- instrumentalVariables( bp1, n, m )
         if( length( iv ) > 0 ){
             cat( n, m, "\n" )
             print( iv , prefix=" * " )}} }
```
...
These are weighted values analysed using a software package called 'survey' @survey2004 . Other software is also used and a complete list included in appendix 3.
...
svyboxplot(omsysval~ agegad2 ,ndns_1_11d, na.rm = TRUE,main = "Systolic BP in mmHg by Age", ylab = "Systolic BP in mmHg" ,xlab ="Age")
```
...
```
The plots appear to show limited correlation.
...
lm1AS <- svyglm(omsysval ~  Sex +bmival +agegad2+educfinh + EIMD_2010_quintile , ndns_1_11d, na.action = na.exclude)

theme_gtsummary_mean_sd()

#sodium only
lmna <- svyglm( omsysval ~ Sodiummg  +Sex +agegad2 + bmival+educfinh+  EIMD_2010_quintile , ndns_1_11d,na.action = na.exclude)

lmnaE <- svyglm(omsysval ~ Sodiummg +Sex +agegad2+Epcnt_4 + bmival +educfinh+ EIMD_2010_quintile , ndns_1_11d,na.action = na.exclude)

lmME <- svyglm( omsysval~ Epcnt_4 +agegad2+Sex + bmival +educfinh + EIMD_2010_quintile , ndns_1_11d,na.action =na.exclude)
...
#sodium only
lmnaj <- svyglm( omsysval ~ Sodiummg + agegad2 , ndns_1_11d,na.action = na.exclude)

lmnaEj <- svyglm(omsysval ~ Sodiummg +Epcnt_4 +agegad2, ndns_1_11d,na.action = na.exclude)

lmMEj <- svyglm( omsysval~ Epcnt_4 +agegad2 , ndns_1_11d,na.action =na.exclude)
#lmnaja  <-      anova(lmnaj)
#lmnaja
```
...
lmnajt <- tbl_regression(lmnaj)
lmnaEjt <- tbl_regression(lmnaEj)
lmMEjt <- tbl_regression(lmMEj)

 as_flex_table(  tbl_merge(list(lmnajt,lmMEjt,lmnaEjt), tab_spanner = c("Sodium only","UPF only","Sodium and UPF")))
```
...
AICjt<- data.table("Model"=c("Sodium only","UPF only","Sodium and UPF"),"AIC" =c( lmnaj$aic,lmMEj$aic,lmnaEj$aic))
flextable(AICjt)
...
The 'best ' of these models is that with only sodium included. The UPF models both being further away from the lowest value.
...
lm1AS2 <- svyglm(Sodiummg ~  Sex +bmival +agegad2+educfinh + EIMD_2010_quintile , ndns_1_11d, na.action = na.exclude)

theme_gtsummary_mean_sd()

#sodium only
lmna2 <- svyglm( Sodiummg ~  omsysval +Sex +agegad2 + bmival+educfinh+  EIMD_2010_quintile , ndns_1_11d,na.action = na.exclude)

lmnaE2 <- svyglm(Sodiummg ~ omsysval +Sex +agegad2+Epcnt_4 + bmival +educfinh+ EIMD_2010_quintile , ndns_1_11d,na.action = na.exclude)

lmME2 <- svyglm( Sodiummg ~  Epcnt_4 +agegad2+Sex + bmival +educfinh + EIMD_2010_quintile , ndns_1_11d,na.action =na.exclude)
...
The baseline regression shows some changes from the previous baseline. The sex coefficent is still statistically significant. The bmi coefficient is not. The age pattern shows an increase in the second age group, but then clear reduction in subsequent ones . That with education and IMD is similar. 

Introducing BP doesn't change the structure of the beta values, or the statistical significance, but the AIC value @tbl-AIC-comparison-Na changes significantly. There is clearly a strong effect of BP on the nature of the regressions. This is shown again when BP and UPF are included.
...
lm1AS3 <- svyglm(Epcnt_4 ~  Sex +bmival +agegad2+educfinh + EIMD_2010_quintile , ndns_1_11d, na.action = na.exclude)

theme_gtsummary_mean_sd()

#sodium only
lmna3 <- svyglm( Epcnt_4 ~  omsysval +Sex +agegad2 + bmival+educfinh+  EIMD_2010_quintile , ndns_1_11d,na.action = na.exclude)

lmnaE3 <- svyglm(Epcnt_4 ~ omsysval +Sex +agegad2+Sodiummg + bmival +educfinh+ EIMD_2010_quintile , ndns_1_11d,na.action = na.exclude)

lmME3 <- svyglm( Epcnt_4 ~  Sodiummg +agegad2+Sex + bmival +educfinh + EIMD_2010_quintile , ndns_1_11d,na.action =na.exclude)

```

```{r tbl-upf-multivariable-outputs, echo=FALSE, message=FALSE, warning=FALSE}
...
lmME3t <- tbl_regression(lmME3)
 ...
 as_flex_table(  tbl_merge(list(lm1AS3t,lmna3t,lmME3t,lmnaE3t), tab_spanner = c("No BP or Na","BP only","Na only","Na and BP")))
```

```{r tbl-AIC-comparison-upf, echo=FALSE, message=FALSE, warning=FALSE}
...
## Data and analysis of change across cohorts
