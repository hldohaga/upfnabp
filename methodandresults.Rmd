...
svyplot(Epcnt_4~SurveyYear,ndns_1_11ed, style = "transparent", na.rm = TRUE)
svyplot(Sodiummg~SurveyYear,ndns_1_11ed)
svyplot(omsysval~SurveyYear,ndns_1_11ed, na.rm = TRUE)
```
...
```{r echo=FALSE}
ttestNa2 <- svyttest(Sodiummg~(SurveyYear >=5), ndns_1_11ed )
ttestNa2
ttestEpcnt <- svyttest(Epcnt_4~(SurveyYear >=5), ndns_1_11ed )
ttestEpcnt
ttestEkJ <- svyttest(EnergykJ_4~(SurveyYear >=5), ndns_1_11ed )
ttestEkJ
...
ttestOS <- svyttest(omsysval~SurveyYear >=5, ndns_1_11ed )
...
#UPF Energy % only
lm1E <- svyglm(omsysval ~ EnergykJ_4, ndns_1_11ed)
...
#linear regression models
#sodium only
lm1na <- svyglm(omsysval ~ Sodiummg, ndns_1_11ed)
...
#linear regression models
#sodium only
lm1AS <- svyglm(omsysval ~ Age + Sex +bmival +educfinh + EIMD_2010_quintile , ndns_1_11ed, na.action = na.exclude)
...
```{r echo=FALSE}
lmM <- svyglm(omsysval ~ Age +Sex + Sodiummg + bmival+ educfinh+ EIMD_2010_quintile , ndns_1_11ed,na.action = na.exclude)
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
lmM2 <- svyglm(omsysval ~ Age +Sex +Sodiummg + EnergykJ_4+ bmival  +educfinh+ EIMD_2010_quintile , ndns_1_11ed,na.action = na.exclude)
...
lmMna <- svyglm(omsysval ~ Age +Sex + EnergykJ_4+ bmival  +educfinh+ EIMD_2010_quintile  , ndns_1_11ed,na.action =na.exclude)
