...
keydatw1 <-tbl_svysummary(ndns_1_11d, include= c(age, agegad3,Sex, omsysval, bmival, Sodiummg, Epcnt_4,UPF3, educfinh, EIMD_2010_quintile,gor ) )
keydatwdd <-tbl_svysummary(ndns_1_11dd, include= c(age, agegad3,Sex, omsysval, bmival, Sodiummg, Epcnt_4,UPF3, educfinh, EIMD_2010_quintile,gor ) )
keydatwT <- tbl_merge(list(keydatw1,keydatwdd))
as_flex_table( keydatwT)
```
...
tablecon <- tbl_stack(list(ttestages,ttestbmis),group_header =  c("Age","BMI"))
tablecon
```
...
#| label: tbl-keydataA
#| tbl-cap: "Table showing the key variables by age group showing differences between exposure and outcome in different age groups"
...
#| label: fig-dag2
#| fig-cap: "diagram of the relationships explored by the analysis"
bp1 <- dagitty(" dag {
               Na -> BP
               Age -> BP
               Sex -> BP
...
AICt<- data.table("Model"=c("BP1:No sodium or UPF","BP1:Sodium only","BP1:Epcnt only","BP1:Sodium and UPF"),"AIC" =c(lm1AS$aic, lmna$aic,lmME$aic,lmnaE$aic))
...
```{r AIC-comparison-AS, echo=FALSE, message=FALSE, warning=FALSE}


AICjt<- data.table("Model"=c("BP:Sodium only","BP:UPF only","BP:Sodium and UPF"),"AIC" =c( lmnaj$aic,lmMEj$aic,lmnaEj$aic))
#AICjt <- flextable(AICjt)
...
```{r AIC-comparison-Na, echo=FALSE, message=FALSE, warning=FALSE}


AIC2t<- data.table("Model"=c("Na:No bp or UPF","Na:bp only","Na:UPF only","Na:bp and UPF"),"AIC" =c(lm1AS2$aic, lmna2$aic,lmME2$aic,lmnaE2$aic))
...
#| tbl-cap: "AIC-comparisonf"

AIC3t<- data.table("Model"=c("UPF:No BP or Na","UPF:BP only","UPF:Na only","UPF:BP and Na"),"AIC" =c(lm1AS3$aic, lmna3$aic,lmME3$aic,lmnaE3$aic))

cfAIC <- rbind(AICt,AICjt,AIC2t,AIC3t)
...
The difference between the two values as a proportion of the larger is `r sense1` %.
