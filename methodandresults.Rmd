...
```{r Na-Sex-Boxplots, echo=FALSE, fig.id=TRUE ,message=FALSE, warning=FALSE}
...
```{r lmMEj-AS6, echo=FALSE, message=FALSE, warning=FALSE}
...
#sodium only
lmnaj <- svyglm( omsysval ~ Sodiummg + Age , ndns_1_11d,na.action = na.exclude)

lmnaEj <- svyglm(omsysval ~ Sodiummg +Epcnt_4 +Age, ndns_1_11d,na.action = na.exclude)

lmMEj <- svyglm( omsysval~ Epcnt_4 +Age , ndns_1_11d,na.action =na.exclude)
#lmnaja  <-      anova(lmnaj)
#lmnaja
```

```{r multivariable-outputsAS, echo=FALSE, message=FALSE, warning=FALSE}
#| label: tbl-multivariable-outputsAS
#| tbl-cap: "multivariable-outputs BP"
...
lmMEjt <- tbl_regression(lmMEj)
 ...
 as_flex_table(  tbl_merge(list(lmnajt,lmMEjt,lmnaEjt), tab_spanner = c("Sodium only","Epcnt only","Sodium and UPF")))
```

```{r AIC-comparison2, echo=FALSE, message=FALSE, warning=FALSE}
#| label: tbl-AIC-comparison-AS
#| tbl-cap: "AIC-comparison AS"

AICjt<- data.table("Model"=c("Sodium only","Epcnt only","Sodium and UPF"),"AIC" =c( lmnaj$aic,lmMEj$aic,lmnaEj$aic))
flextable(AICjt)
...
```{r AIC-comparison3, echo=FALSE, message=FALSE, warning=FALSE}
#| label: tbl-AIC-comparison-Na
#| tbl-cap: "AIC-comparison Na"

AIC2t<- data.table("Model"=c("No bp or UPF","bp only","Epcnt only","bp and UPF"),"AIC" =c(lm1AS2$aic, lmna2$aic,lmME2$aic,lmnaE2$aic))
flextable(AIC2t)
```
