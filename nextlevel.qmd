...
These participants were excluded however this affected the sample size and skewed the male-female ratio, the overall results were unchanged. They have been included in multivariable analysis and conducted a sensitivity analysis on these participants taking BP medications.
...
Nova group 4 or UPF intake (Epcnt_4, UPF) is used for this study.

A variable (UPF3)  was developed from the mean UPF intake. The central category is the mean with one standard deviation above and below. This effectively identifies 67% in the centre of the distribution. 

A second variable UPF2 was derived from this. This identifies those more than one standard deviation above the mean.
...
ndns_1_11$hiNa <- cut(ndns_1_11$Sodiummg , breaks = c(0,3000,10000))
```


```{r taking-any-meds, echo=FALSE, message=FALSE, warning=FALSE}
...
#| label: tbl-keydata1
#| tbl-cap: "Table of variables used in analysis.Continuous variables are represented by the mean and standard deviation in brackets.Categorical variables give the number of participants and the percentage of the sample in brackets."
...
#keydatwdd <-tbl_svysummary(ndns_1_11d$sex=="female", include= c(age, agegad3,Sex, omsysval, bmival, Sodiummg, Epcnt_4,UPF3, educfinh, EIMD_2010_quintile,gor ) )
...
lmnaE <- svyglm(hyp ~ hiNa +Sex +agegad3+UPF2 + bmival +educfinh+ EIMD_2010_quintile +bpd ,family = binomial(link = "logit"), ndns_1_11d,na.action = na.exclude)

lmME <- svyglm( hyp~ UPF2 +agegad3+Sex + bmival +educfinh + EIMD_2010_quintile +bpd,family = binomial(link = "logit"), ndns_1_11d,na.action =na.exclude)
...
#sodium only
lmnaj <- svyglm( hyp ~ Sodiummg + agegad3 +bpd, ndns_1_11d,family = binomial(link = "logit"),na.action = na.exclude)

lmnaEj <- svyglm(hyp ~ Sodiummg +Epcnt_4 +agegad3+bpd,family = binomial(link = "logit"), ndns_1_11d,na.action = na.exclude)

lmMEj <- svyglm( hyp~ Epcnt_4 +agegad3 +bpd,family = binomial(link = "logit"), ndns_1_11d,na.action =na.exclude)
#lmnaja  <-      anova(lmnaj)
#lmnaja
```
