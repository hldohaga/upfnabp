...
The data is read from its files using 'r-studio' with the processing being carried out using packages available from CRAN `r citation()` .
...
```{r keydata, echo=FALSE, message=FALSE, warning=FALSE,tab.id=TRUE}
#| label: tbl-keydata
#| tbl-cap: "key data"
...
ttestage <- svyglm(Age ~ SurveyYear, ndns_1_11c)
ttestages<-  tbl_regression(ttestage)


ttestbmi <- svyglm(bmival~SurveyYear, ndns_1_11c)
...
These values in table @tbl-Categorical-data) identify a significant difference in the number of vegetarians
...
sexchi <- svychisq(~Sex +SurveyYear , ndns_1_11c)
educhi <- svychisq(~educfinh+SurveyYear , ndns_1_11c)
imdchi <- svychisq(~EIMD_2010_quintile+SurveyYear , ndns_1_11c)
vegchi <- svychisq(~vegetarn+ SurveyYear , ndns_1_11c)
...
First, omsysval is compared to EnergykJ, then sodiummg in this table @tbl-univariable-regressions
...
@tbl-keydata with summary values for the key variables across the dataset showing a change across the cohorts.
This is less evident in the plots.

@tbl-Key-Variables-by-Survey-year shows statistical analysis of the key variables shows the change across the cohorts.

@tbl-continuous-data and @tbl-Categorical-variables-year show that other variables are analysed and show that there has been change in the balance of the populations.

Univariable regression, @tbl-univariable-regressions , shows a degree of association between the BP and UPF intake.
It also shows the same for sodium intake.
It also shows that the change with age is the same size as the change for UPF.

Using Anova analysis of different multi variable regression models, @tbl-multivariable-outputs-bp , the key variables are significant for sodium in several models with age included and sometimes for UPF with age excluded.
...
There is a correlation between Age and UPF intake.
...
I will now go on to discuss the meaning of these results in the discussion section.
